...
STM8TargetLowering::STM8TargetLowering(const TargetMachine &TM,
  ...
  if (STI.hasHWMult16()) {
    ...
    setLibcallCallingConv(RTLIB::MUL_I64, CallingConv::STM8_BUILTIN);
  }

  // Several of the runtime library functions use a special calling conv
  setLibcallCallingConv(RTLIB::UDIV_I64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::UREM_I64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::SDIV_I64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::SREM_I64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::ADD_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::SUB_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::MUL_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::DIV_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::OEQ_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::UNE_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::OGE_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::OLT_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::OLE_F64, CallingConv::STM8_BUILTIN);
  setLibcallCallingConv(RTLIB::OGT_F64, CallingConv::STM8_BUILTIN);
...
//===----------------------------------------------------------------------===//
//                       STM8 Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TargetLowering::ConstraintType
STM8TargetLowering::getConstraintType(StringRef Constraint) const {
...
STM8TargetLowering::getRegForInlineAsmConstraint(
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':   // GENERAL_REGS
      if (VT == MVT::i8)
        return std::make_pair(0U, &STM8::GR8RegClass);

      return std::make_pair(0U, &STM8::GR16RegClass);
    }
  }
...
static void AnalyzeVarArgs(CCState &State,
  ...
  State.AnalyzeCallOperands(Outs, CC_STM8_AssignStack);
}

static void AnalyzeVarArgs(CCState &State,
                           const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeFormalArguments(Ins, CC_STM8_AssignStack);
}
...
static void AnalyzeArguments(CCState &State,
  ...
  bool Builtin = (State.getCallingConv() == CallingConv::STM8_BUILTIN);
  if (Builtin) {
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UsedStack && Parts == 2 && RegsLeft == 1) {
      ...
      UsedStack = true;
      CC_STM8_AssignStack(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
...
static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeCallResult(Ins, RetCC_STM8);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs) {
  State.AnalyzeReturn(Outs, RetCC_STM8);
}
...
SDValue
STM8TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // STM8 target does not yet support tail call optimization.
  isTailCall = false;

  switch (CallConv) {
  default:
    report_fatal_error("Unsupported calling convention");
  case CallingConv::STM8_BUILTIN:
  case CallingConv::Fast:
  case CallingConv::C:
...
SDValue STM8TargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  STM8MachineFunctionInfo *FuncInfo = MF.getInfo<STM8MachineFunctionInfo>();
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i16:
        unsigned VReg = RegInfo.createVirtualRegister(&STM8::GR16RegClass);
...
bool
STM8TargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                     MachineFunction &MF,
                                     bool IsVarArg,
                                     const SmallVectorImpl<ISD::OutputArg> &Outs,
...
STM8TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  if (CallConv == CallingConv::STM8_INTR && !Outs.empty())
  ...
  unsigned Opc = (CallConv == CallingConv::STM8_INTR ?
                  STM8ISD::RETI_FLAG : STM8ISD::RET_FLAG);
...
SDValue STM8TargetLowering::LowerCCCCallTo(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, dl, STM8::SP, PtrVT);
  ...
  Chain = DAG.getNode(STM8ISD::CALL, dl, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
SDValue STM8TargetLowering::LowerShifts(SDValue Op,
                                          SelectionDAG &DAG) const {
  ...
  if (!isa<ConstantSDNode>(N->getOperand(1)))
    ...
    case ISD::SHL:
      return DAG.getNode(STM8ISD::SHL, dl,
                         VT, N->getOperand(0), N->getOperand(1));
    case ISD::SRA:
      return DAG.getNode(STM8ISD::SRA, dl,
                         VT, N->getOperand(0), N->getOperand(1));
    case ISD::SRL:
      return DAG.getNode(STM8ISD::SRL, dl,
                         VT, N->getOperand(0), N->getOperand(1));
    }
  ...
  if (Opc == ISD::SRL && ShiftAmount) {
    ...
    // srl A, 1 => clrc; rrc A
    Victim = DAG.getNode(STM8ISD::RRC, dl, VT, Victim);
    ShiftAmount -= 1;
  }

  while (ShiftAmount--)
    Victim = DAG.getNode((Opc == ISD::SHL ? STM8ISD::RLA : STM8ISD::RRA),
                         dl, VT, Victim);
...
SDValue STM8TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(STM8ISD::Wrapper, SDLoc(Op), PtrVT, Result);
}

SDValue STM8TargetLowering::LowerExternalSymbol(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc dl(Op);
  ...
  return DAG.getNode(STM8ISD::Wrapper, dl, PtrVT, Result);
}

SDValue STM8TargetLowering::LowerBlockAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc dl(Op);
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  STM8CC::CondCodes TCC = STM8CC::COND_INVALID;
  switch (CC) {
  default: llvm_unreachable("Invalid integer condition!");
  case ISD::SETEQ:
    TCC = STM8CC::COND_E;     // aka COND_Z
    ...
    break;
  case ISD::SETNE:
    TCC = STM8CC::COND_NE;    // aka COND_NZ
    // Minor optimization: if LHS is a constant, swap operands, then the
    // constant can be folded into comparison.
    if (LHS.getOpcode() == ISD::Constant)
      std::swap(LHS, RHS);
  ...
  case ISD::SETULT:
    ...
    TCC = STM8CC::COND_LO;    // aka COND_NC
    break;
  case ISD::SETLE:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  ...
  return DAG.getNode(STM8ISD::CMP, dl, MVT::Glue, LHS, RHS);
}
...
SDValue STM8TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(STM8ISD::BR_CC, dl, Op.getValueType(),
                     Chain, Dest, TargetCC, Flag);
}

SDValue STM8TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (Convert) {
    SDValue SR = DAG.getCopyFromReg(DAG.getEntryNode(), dl, STM8::SR,
                                    MVT::i16, Flag);
    if (Shift)
    ...
    return DAG.getNode(STM8ISD::SELECT_CC, dl, VTs, Ops);
  }
}
...
SDValue STM8TargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue
STM8TargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  STM8MachineFunctionInfo *FuncInfo = MF.getInfo<STM8MachineFunctionInfo>();
...
SDValue STM8TargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue STM8TargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue STM8TargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  STM8MachineFunctionInfo *FuncInfo = MF.getInfo<STM8MachineFunctionInfo>();
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue STM8TargetLowering::LowerJumpTable(SDValue Op,
    ...
    return DAG.getNode(STM8ISD::Wrapper, SDLoc(JT), PtrVT, Result);
}

/// getPostIndexedAddressParts - returns true by value, base pointer and
/// offset pointer and addressing mode by reference if this node can be
/// combined with a load / store to form a post-indexed load / store.
bool STM8TargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                      SDValue &Base,
                                                      SDValue &Offset,
...
const char *STM8TargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case STM8ISD::RETI_FLAG:          return "STM8ISD::RETI_FLAG";
  case STM8ISD::RRA:                return "STM8ISD::RRA";
  case STM8ISD::RLA:                return "STM8ISD::RLA";
  case STM8ISD::RRC:                return "STM8ISD::RRC";
  case STM8ISD::CALL:               return "STM8ISD::CALL";
  case STM8ISD::Wrapper:            return "STM8ISD::Wrapper";
  case STM8ISD::BR_CC:              return "STM8ISD::BR_CC";
  case STM8ISD::CMP:                return "STM8ISD::CMP";
  case STM8ISD::SETCC:              return "STM8ISD::SETCC";
  case STM8ISD::SELECT_CC:          return "STM8ISD::SELECT_CC";
  case STM8ISD::SHL:                return "STM8ISD::SHL";
  case STM8ISD::SRA:                return "STM8ISD::SRA";
  case STM8ISD::SRL:                return "STM8ISD::SRL";
  }
  return nullptr;
}

bool STM8TargetLowering::isTruncateFree(Type *Ty1,
                                          Type *Ty2) const {
...
bool STM8TargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool STM8TargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // STM8 implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool STM8TargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // STM8 implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && VT1 == MVT::i8 && VT2 == MVT::i16;
}

bool STM8TargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
MachineBasicBlock *
STM8TargetLowering::EmitShiftInstr(MachineInstr &MI,
  ...
  unsigned ShiftAmtReg = RI.createVirtualRegister(&STM8::GR8RegClass);
  unsigned ShiftAmtReg2 = RI.createVirtualRegister(&STM8::GR8RegClass);
  ...
  // cmp 0, N
  // je RemBB
  BuildMI(BB, dl, TII.get(STM8::CMP8ri))
    .addReg(ShiftAmtSrcReg).addImm(0);
  BuildMI(BB, dl, TII.get(STM8::JCC))
    .addMBB(RemBB)
    .addImm(STM8CC::COND_E);
  ...
  BuildMI(LoopBB, dl, TII.get(STM8::PHI), ShiftReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(STM8::PHI), ShiftAmtReg)
    .addReg(ShiftAmtSrcReg).addMBB(BB)
    .addReg(ShiftAmtReg2).addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(Opc), ShiftReg2)
    .addReg(ShiftReg);
  BuildMI(LoopBB, dl, TII.get(STM8::SUB8ri), ShiftAmtReg2)
    .addReg(ShiftAmtReg).addImm(1);
  BuildMI(LoopBB, dl, TII.get(STM8::JCC))
    .addMBB(LoopBB)
    .addImm(STM8CC::COND_NE);

  // RemBB:
  // DestReg = phi [%SrcReg, BB], [%ShiftReg, LoopBB]
  BuildMI(*RemBB, RemBB->begin(), dl, TII.get(STM8::PHI), DstReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
...
STM8TargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  if (Opc == STM8::Shl8 || Opc == STM8::Shl16 ||
      Opc == STM8::Sra8 || Opc == STM8::Sra16 ||
      Opc == STM8::Srl8 || Opc == STM8::Srl16)
    return EmitShiftInstr(MI, BB);

  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();

  assert((Opc == STM8::Select16 || Opc == STM8::Select8) &&
  ...
  //  ...
  BB = copy1MBB;
  BuildMI(*BB, BB->begin(), dl, TII.get(STM8::PHI), MI.getOperand(0).getReg())
