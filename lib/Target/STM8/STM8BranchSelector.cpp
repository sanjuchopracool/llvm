//===-- STM8BranchSelector.cpp - Emit long conditional branches ---------===//
//
...
#define DEBUG_TYPE "stm8-branch-select"

static cl::opt<bool>
    BranchSelectEnabled("stm8-branch-select", cl::Hidden, cl::init(true),
...
namespace {
class STM8BSel : public MachineFunctionPass {
...
public:
  static char ID;
  STM8BSel() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return "STM8 Branch Selector"; }
};
char STM8BSel::ID = 0;
}
...
unsigned STM8BSel::measureFunction(OffsetVector &BlockOffsets,
...
bool STM8BSel::expandBranches(OffsetVector &BlockOffsets) {
  ...
  for (auto MBB = MF->begin(), E = MF->end(); MBB != E; ++MBB) {
    ...
    for (auto MI = MBB->begin(), EE = MBB->end(); MI != EE; ++MI) {
      ...
      if (MI->getOpcode() != STM8::JCC && MI->getOpcode() != STM8::JMP) {
        continue;
      }
      ...
      if (MI->getOpcode() == STM8::JCC && std::next(MI) != EE) {
      ...
      MI = BuildMI(*MBB, MI, dl, TII->get(STM8::Bi)).addMBB(DestBB);
      InstrSizeDiff += TII->getInstSizeInBytes(*MI);
...
bool STM8BSel::runOnMachineFunction(MachineFunction &mf) {
  MF = &mf;
  TII = static_cast<const STM8InstrInfo *>(MF->getSubtarget().getInstrInfo());
...
FunctionPass *llvm::createSTM8BranchSelectionPass() {
  return new STM8BSel();
}
