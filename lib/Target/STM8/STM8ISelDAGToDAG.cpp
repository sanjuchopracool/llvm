...
namespace {
  ...
  public:
    ...
    bool MatchAddress(SDValue N, STM8ISelAddressMode &AM);
    bool MatchWrapper(SDValue N, STM8ISelAddressMode &AM);
    bool MatchAddressBase(SDValue N, STM8ISelAddressMode &AM);
...
/// MatchWrapper - Try to match STM8ISD::Wrapper node into an addressing mode.
/// These wrap things that will resolve down into a symbol reference.  If no
/// match is possible, this returns true, otherwise it returns false.
bool STM8DAGToDAGISel::MatchWrapper(SDValue N, STM8ISelAddressMode &AM) {
...
bool STM8DAGToDAGISel::MatchAddressBase(SDValue N, STM8ISelAddressMode &AM) {
  // Is the base register already occupied?
  if (AM.BaseType != STM8ISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = STM8ISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
}

bool STM8DAGToDAGISel::MatchAddress(SDValue N, STM8ISelAddressMode &AM) {
  ...
  case STM8ISD::Wrapper:
    if (!MatchWrapper(N, AM))
      return false;
    break;

  case ISD::FrameIndex:
    if (AM.BaseType == STM8ISelAddressMode::RegBase
        && AM.Base.Reg.getNode() == nullptr) {
      AM.BaseType = STM8ISelAddressMode::FrameIndexBase;
...
bool STM8DAGToDAGISel::SelectAddr(SDValue N,
  ...
  Base = (AM.BaseType == STM8ISelAddressMode::FrameIndexBase)
...
bool STM8DAGToDAGISel::tryIndexedLoad(SDNode *N) {
  ...
  switch (VT.SimpleTy) {
  case MVT::i8:
    Opcode = STM8::MOV8rm_POST;
    break;
  case MVT::i16:
    Opcode = STM8::MOV16rm_POST;
    break;
  default:
    return false;
  }
...
bool STM8DAGToDAGISel::tryIndexedBinOp(SDNode *Op, SDValue N1, SDValue N2,
...
void STM8DAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse()) {
      CurDAG->SelectNodeTo(Node, STM8::ADDframe, MVT::i16, TFI,
                           CurDAG->getTargetConstant(0, dl, MVT::i16));
      return;
    }
    ReplaceNode(Node, CurDAG->getMachineNode(
                          STM8::ADDframe, dl, MVT::i16, TFI,
                          CurDAG->getTargetConstant(0, dl, MVT::i16)));
    return;
  }
  case ISD::LOAD:
  ...
  case ISD::ADD:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        STM8::ADD8rm_POST, STM8::ADD16rm_POST))
      return;
  ...
  case ISD::SUB:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        STM8::SUB8rm_POST, STM8::SUB16rm_POST))
      return;
  ...
  case ISD::AND:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        STM8::AND8rm_POST, STM8::AND16rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             STM8::AND8rm_POST, STM8::AND16rm_POST))
      return;
  ...
  case ISD::OR:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        STM8::OR8rm_POST, STM8::OR16rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             STM8::OR8rm_POST, STM8::OR16rm_POST))
      return;
  ...
  case ISD::XOR:
    if (tryIndexedBinOp(Node, Node->getOperand(0), Node->getOperand(1),
                        STM8::XOR8rm_POST, STM8::XOR16rm_POST))
      return;
    else if (tryIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                             STM8::XOR8rm_POST, STM8::XOR16rm_POST))
      return;
