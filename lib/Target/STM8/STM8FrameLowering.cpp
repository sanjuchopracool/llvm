...
void STM8FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  STM8MachineFunctionInfo *STM8FI = MF.getInfo<STM8MachineFunctionInfo>();
  const STM8InstrInfo &TII =
      *static_cast<const STM8InstrInfo *>(MF.getSubtarget().getInstrInfo());
  ...
  if (hasFP(MF)) {
    ...
    NumBytes = FrameSize - STM8FI->getCalleeSavedFrameSize();
    ...
    BuildMI(MBB, MBBI, DL, TII.get(STM8::PUSH16r))
      .addReg(STM8::FP, RegState::Kill);

    // Update FP with the new base value...
    BuildMI(MBB, MBBI, DL, TII.get(STM8::MOV16rr), STM8::FP)
      .addReg(STM8::SP);
    ...
    NumBytes = StackSize - STM8FI->getCalleeSavedFrameSize();

  // Skip the callee-saved push instructions.
  while (MBBI != MBB.end() && (MBBI->getOpcode() == STM8::PUSH16r))
    ++MBBI;
  ...
  if (NumBytes) { // adjust stack pointer: SP -= numbytes
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(STM8::SUB16ri), STM8::SP)
        .addReg(STM8::SP).addImm(NumBytes);
...
void STM8FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  switch (RetOpcode) {
  case STM8::RET:
  case STM8::RETI: break;  // These are ok
  default:
  ...
  // Get the number of bytes to allocate from the FrameInfo
  uint64_t StackSize = MFI.getStackSize();
  unsigned CSSize = STM8FI->getCalleeSavedFrameSize();
  uint64_t NumBytes = 0;
  ...
  if (hasFP(MF)) {
    ...
    // pop FP.
    BuildMI(MBB, MBBI, DL, TII.get(STM8::POP16r), STM8::FP);
  } else
  ...
  while (MBBI != MBB.begin()) {
    ...
    if (Opc != STM8::POP16r && !PI->isTerminator())
      break;
    --MBBI;
  }
  ...
  if (MFI.hasVarSizedObjects()) {
    BuildMI(MBB, MBBI, DL,
            TII.get(STM8::MOV16rr), STM8::SP).addReg(STM8::FP);
    if (CSSize) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL,
                TII.get(STM8::SUB16ri), STM8::SP)
        .addReg(STM8::SP).addImm(CSSize);
      // The SRW implicit def is dead.
      MI->getOperand(3).setIsDead();
    }
  } else {
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(STM8::ADD16ri), STM8::SP)
        .addReg(STM8::SP).addImm(NumBytes);
      // The SRW implicit def is dead.
      MI->getOperand(3).setIsDead();
    }
...
STM8FrameLowering::spillCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  STM8MachineFunctionInfo *MFI = MF.getInfo<STM8MachineFunctionInfo>();
  ...
  for (unsigned i = CSI.size(); i != 0; --i) {
    ...
    MBB.addLiveIn(Reg);
    BuildMI(MBB, MI, DL, TII.get(STM8::PUSH16r))
...
STM8FrameLowering::restoreCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  for (unsigned i = 0, e = CSI.size(); i != e; ++i)
    BuildMI(MBB, MI, DL, TII.get(STM8::POP16r), CSI[i].getReg());
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (Old.getOpcode() == TII.getCallFrameSetupOpcode()) {
        New =
            BuildMI(MF, Old.getDebugLoc(), TII.get(STM8::SUB16ri), STM8::SP)
                .addReg(STM8::SP)
                .addImm(Amount);
      } else {
        ...
        if (Amount)
          New = BuildMI(MF, Old.getDebugLoc(), TII.get(STM8::ADD16ri),
                        STM8::SP)
                    .addReg(STM8::SP)
                    .addImm(Amount);
      }
